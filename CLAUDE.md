...
- **Merge gate:** never merge a PR until **(a)** CI is green — every configured check passing (`gh pr checks`), never over a pending or failing one — **and (b)** Devin's automated review has posted (Devin auto-reviews every PR, and its review also registers as a check — don't merge one it hasn't reviewed yet). When the review lands, move the Linear issue to **Under Review** and work its findings there: address _every_ issue Devin raises, replying to each finding individually on the PR (fix + resolve the thread, or a reasoned decline), push fixes, let CI re-run green, and record the findings + resolutions as a comment on the issue. Only then `gh pr merge --merge`. **Convention only** — no repo-side hook enforces this; don't shortcut it. Mirrors `askara-community`'s canonical rule — [its CLAUDE.md § Rules](https://github.com/Askara-Solutions/askara-community/blob/main/CLAUDE.md#rules), with the step-by-step boxes in that repo's `agent_docs/checklists.md § Before Merging`.
